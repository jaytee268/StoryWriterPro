...
export type CharacterMemoryStatus = 'proposed' | 'confirmed' | 'rejected' | 'retired' | 'retconned';
...
export interface CharacterKnowledgeState { id: string; projectId: string; characterId: string; factEntityId: string; knowledgeState: CharacterKnowledgeKind; acquiredSceneId?: string; changedSceneId?: string; sourceCharacterId?: string; certainty: number; notes: string; status: CharacterMemoryStatus; authorConfirmed: boolean; createdAt: string; updatedAt: string; }
...
export interface CharacterMemoryUpdateRun { id: string; projectId: string; sceneId: string; contentHash: string; extractorId: string; status: BibleRunStatus; createdAt: string; completedAt?: string; errorMessage?: string; }
export type CharacterMemoryPayload = Record<string, string | number | boolean | null | undefined>;
export interface CharacterMemoryProposal { id: string; runId: string; projectId: string; sceneId: string; proposalKind: CharacterMemoryProposalKind; subjectCharacterId?: string; relatedCharacterId?: string; targetEntityId?: string; payload: CharacterMemoryPayload; classification: CharacterMemoryClassification; confidence: number; evidenceExcerpt: string; startOffset?: number; endOffset?: number; reason: string; reviewStatus: BibleReviewStatus; reviewedAt?: string; createdAt: string; }
...
export interface CreateCharacterMemoryUpdateRunInput { projectId: string; sceneId: string; contentHash: string; extractorId: string; }
export interface ReviewCharacterMemoryProposalInput { proposalId: string; reviewStatus: Exclude<BibleReviewStatus, 'pending'>; payload?: CharacterMemoryPayload; }
...
export interface DraftContinuityState { currentLocation: string; currentStoryTime: string; presentCharacterIds: string[]; characterStates: Record<string, string>[]; establishedFacts: string[]; knowledgeChanges: PlannedKnowledgeChange[]; relationshipChanges: PlannedRelationshipChange[]; movedObjects: Record<string, string>[]; injuries: Record<string, string>[]; cluesIntroduced: string[]; promisesCreated: string[]; unresolvedActions: string[]; lastParagraphSummary: string; }
