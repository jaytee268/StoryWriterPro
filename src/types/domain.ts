...
export interface Book { id: string; projectId: string; title: string; volume: number; primaryGenreId?: string; secondaryGenreIds: string[]; customGenreNames: string[]; genreSource?: BookGenreSource; genreConfidence?: number; genreReason?: string; genreAuthorConfirmed: boolean; genreDetectedAt?: string; genreSupportingSignals?: string[]; genreContradictingSignals?: string[]; genreAlternatives?: Array<{ genreId?: string; customGenre?: string; reason?: string }>; genreAudienceNotes?: string[]; genreWarnings?: string[]; genrePromptVersion?: string; createdAt?: string; updatedAt?: string; }
export interface SaveBookGenresInput { bookId: string; projectId: string; primaryGenreId?: string; secondaryGenreIds: string[]; customGenreNames: string[]; genreSource?: BookGenreSource; genreConfidence?: number; genreReason?: string; genreAuthorConfirmed: boolean; genreDetectedAt?: string; genreSupportingSignals?: string[]; genreContradictingSignals?: string[]; genreAlternatives?: Array<{ genreId?: string; customGenre?: string; reason?: string }>; genreAudienceNotes?: string[]; genreWarnings?: string[]; genrePromptVersion?: string; analysisJobId?: string; providerId?: string; }
export interface Chapter { id: string; bookId: string; title: string; orderIndex: number; scenes: Scene[]; importVersionId?: string; importStatus?: 'active' | 'archived'; createdAt?: string; updatedAt?: string; }
...
export interface BibleExtractionInput { project: Project; chapter: Chapter; scene: Scene; existingEntities: StoryEntity[]; provisionalEntities?: ProvisionalEntity[]; provisionalAliases?: ProvisionalAlias[]; relevantSources?: StorySourceReference[]; previousAnalyzedContent?: string; changedRange?: { start: number; end: number }; }
...
export interface ProjectRuleProposal { id: string; projectId: string; targetRuleId?: string; title: string; statement: string; scope: ProjectRuleScope; prerequisites: string[]; effects: string[]; exceptions: string[]; connectedLoreIds: string[]; sourceReferenceIds: string[]; evidenceExcerpt: string; chapterId?: string; sceneId?: string; startOffset?: number; endOffset?: number; confidence: number; reason: string; reviewStatus: ProjectRuleProposalReviewStatus; reviewedAt?: string; createdAt: string; }
export interface SaveProjectRuleProposalInput extends Omit<ProjectRuleProposal, 'id' | 'createdAt' | 'reviewedAt' | 'reviewStatus'> { id?: string; reviewStatus?: ProjectRuleProposalReviewStatus; }
export type ContinuityStateKind = 'item_existence' | 'item_availability' | 'ownership' | 'location' | 'physical_condition' | 'injury' | 'property' | 'knowledge' | 'relationship' | 'promise' | 'goal' | 'open_action';
export type ContinuityStateStatus = 'proposed' | 'confirmed' | 'uncertain' | 'contradicted' | 'rejected' | 'retconned';
export interface ManuscriptPosition { chapterId: string; sceneId: string; offset?: number; }
export interface ContinuityStateLedgerEntry { id: string; projectId: string; entityId: string; relatedEntityId?: string; stateKind: ContinuityStateKind; previousState: string; newState: string; reason?: string; evidenceExcerpt?: string; chapterId?: string; sceneId?: string; startOffset?: number; endOffset?: number; sourceReferenceId?: string; status: ContinuityStateStatus; confidence: number; authorConfirmed: boolean; createdAt: string; updatedAt: string; }
export interface SaveContinuityStateInput extends Omit<ContinuityStateLedgerEntry, 'id' | 'createdAt' | 'updatedAt'> { id?: string; }
export type ContinuityReviewFindingType = 'critical_contradiction' | 'probable_contradiction' | 'missing_explanation' | 'character_deviation' | 'lore_compatible_anomaly' | 'possible_intentional_exception' | 'insufficient_evidence';
export type ContinuityReviewSeverity = 'info' | 'warning' | 'critical';
export type ContinuityReviewStatus = 'open' | 'accepted' | 'dismissed' | 'resolved' | 'deferred';
export type ContinuityFindingDecisionStatus = 'open' | 'resolved_after_text_change' | 'resolved_with_confirmed_rule' | 'accepted_exception' | 'deferred_rule_review' | 'deferred_canon_review' | 'deferred_open_question' | 'dismissed';
export type ContinuityFindingDecisionKind = 'text_correction' | 'confirmed_rule' | 'new_rule' | 'canon_review' | 'intentional_exception' | 'open_question' | 'dismiss';
export interface ContinuityFindingDecision { id: string; findingId: string; projectId: string; status: ContinuityFindingDecisionStatus; decisionKind: ContinuityFindingDecisionKind; ruleId?: string; ruleProposalId?: string; openQuestionEntityId?: string; sourceReferenceId?: string; exceptionReason?: string; contentHash?: string; payload?: Record<string, unknown>; createdAt: string; updatedAt: string; }
export interface ContinuityCanonChangeAudit { id: string; findingId: string; projectId: string; targetEntityId?: string; targetStateId?: string; action: 'previous_incomplete' | 'retcon' | 'new_information' | 'unreliable_perspective' | 'cancelled'; reason: string; previousSourceReferenceId?: string; newSourceReferenceId?: string; sourceReferenceIds: string[]; payload?: Record<string, unknown>; createdAt: string; }
export type ContinuityReviewSourceKind = 'word_threshold' | 'page_marker' | 'bible_update' | 'longform_section' | 'manual';
export interface ContinuityReviewSettings { projectId: string; wordThreshold: number; updatedAt: string; }
export type ContinuityReviewRunStatus = 'pending' | 'running' | 'completed' | 'failed' | 'cancelled' | 'reviewed';
export interface ContinuityReviewRun { id: string; projectId: string; chapterId?: string; sceneId?: string; sourceKind: ContinuityReviewSourceKind; contentHash: string; startOffset?: number; endOffset?: number; providerId: string; status: ContinuityReviewRunStatus; createdAt: string; completedAt?: string; errorMessage?: string; }
export interface ContinuityCounterEvidence { sourceReferenceId?: string; excerpt: string; chapterId?: string; sceneId?: string; startOffset?: number; endOffset?: number; }
export interface ContinuityReviewFinding { id: string; runId: string; projectId: string; chapterId?: string; sceneId?: string; findingType: ContinuityReviewFindingType; severity: ContinuityReviewSeverity; subjectEntityId?: string; relatedEntityIds: string[]; relatedStateIds: string[]; relatedRuleIds: string[]; objectiveConflict: string; loreExplanations: string[]; evidenceExcerpt: string; sourceReferenceId?: string; counterEvidenceExcerpts: string[]; counterEvidence?: ContinuityCounterEvidence[]; confidence: number; startOffset?: number; endOffset?: number; reason: string; reviewStatus: ContinuityReviewStatus; userDecision?: string; createdAt: string; updatedAt: string; }
export type PlotThreadLifecycleStatus = 'open' | 'closure_candidate' | 'partially_resolved' | 'resolved' | 'reopened' | 'abandoned';
export interface PlotThreadLifecycle { id: string; projectId: string; entityId: string; lifecycleStatus: PlotThreadLifecycleStatus; lastSourceReferenceId?: string; updatedAt: string; }
export interface PlotThreadLifecycleProposal { id: string; runId: string; projectId: string; entityId: string; proposedStatus: Exclude<PlotThreadLifecycleStatus, 'resolved'>; evidenceExcerpt: string; sourceReferenceId?: string; startOffset?: number; endOffset?: number; reason: string; confidence: number; reviewStatus: BibleReviewStatus; reviewedAt?: string; createdAt: string; }
export interface SaveContinuityReviewInput { projectId: string; chapterId?: string; sceneId?: string; sourceKind: ContinuityReviewSourceKind; contentHash: string; startOffset?: number; endOffset?: number; providerId?: string; }
export interface SaveContinuityReviewRunStatusInput { id: string; status: ContinuityReviewRunStatus; errorMessage?: string; completedAt?: string; }
export interface SaveContinuityFindingInput extends Omit<ContinuityReviewFinding, 'id' | 'createdAt' | 'updatedAt' | 'reviewStatus' | 'userDecision'> { id?: string; reviewStatus?: ContinuityReviewStatus; userDecision?: string; }
export interface ApplyContinuityFindingDecisionInput { findingId: string; projectId: string; status: ContinuityFindingDecisionStatus; decisionKind: ContinuityFindingDecisionKind; ruleId?: string; ruleProposalId?: string; openQuestionEntityId?: string; sourceReferenceId?: string; exceptionReason?: string; contentHash?: string; payload?: Record<string, unknown>; canonAction?: ContinuityCanonChangeAudit['action']; canonTargetEntityId?: string; canonTargetStateId?: string; canonReason?: string; canonSourceReferenceIds?: string[]; }
export interface ReconcileContinuityTextCorrectionInput { projectId: string; sceneId: string; runId: string; contentHash: string; findings: Array<{ findingType: ContinuityReviewFindingType; subjectEntityId?: string; objectiveConflict: string }>; }
export interface ContinuityObservedPassage { summary: string; evidenceExcerpt: string; entityIds: string[]; startOffset?: number; endOffset?: number; }
export interface ContinuityStateChangeProposal { entityId: string; relatedEntityId?: string; stateKind: ContinuityStateKind; previousState: string; newState: string; confidence: number; evidenceExcerpt: string; sourceReferenceId?: string; startOffset?: number; endOffset?: number; reason: string; }
export interface ContinuityContradictionProposal { findingType: Exclude<ContinuityReviewFindingType, 'character_deviation' | 'insufficient_evidence'>; subjectEntityId?: string; relatedEntityIds: string[]; relatedStateIds: string[]; objectiveConflict: string; evidenceExcerpt: string; sourceReferenceId?: string; counterEvidenceExcerpts: string[]; counterEvidence?: ContinuityCounterEvidence[]; confidence: number; startOffset?: number; endOffset?: number; reason: string; }
...
export interface ManuscriptStructureRun { id: string; projectId: string; chapterId: string; contentHash: string; providerId: string; promptVersion: string; status: 'pending' | 'running' | 'completed' | 'failed' | 'reviewed'; errorMessage?: string; createdAt: string; updatedAt: string; }
export interface ManuscriptStructureInput { projectId: string; chapter: Chapter; pageMarkers: ManuscriptAnalysisPageMarker[]; localHints: Array<{ startOffset: number; endOffset: number; reason: string }>; confirmedLore?: LoreMetadata[]; confirmedRules?: ProjectRule[]; }
export interface ManuscriptStructureResult { scenes: Array<Omit<ManuscriptStructureProposal, 'id' | 'runId' | 'projectId' | 'reviewStatus' | 'createdAt' | 'updatedAt'>>; warnings: string[]; }
export interface CreateManuscriptStructureRunInput { projectId: string; chapterId: string; contentHash: string; providerId: string; promptVersion: string; }
export interface SaveManuscriptStructureProposalInput extends Omit<ManuscriptStructureProposal, 'id' | 'createdAt' | 'updatedAt' | 'reviewStatus'> { id?: string; reviewStatus?: ManuscriptStructureReviewStatus; }
export type ProvisionalEntityType = 'character' | 'place' | 'organization' | 'object' | 'event' | 'fact' | 'clue' | 'secret' | 'plot_thread' | 'open_question' | 'world_rule_candidate' | 'author_note';
export type ProvisionalReviewStatus = 'proposed' | 'accepted' | 'rejected' | 'uncertain' | 'merged';
export interface ProvisionalEntity { id: string; jobId: string; projectId: string; entityType: ProvisionalEntityType; canonicalName: string; aliases: string[]; description: string; firstSourceReferenceId?: string; lastSourceReferenceId?: string; confidence: number; reviewStatus: ProvisionalReviewStatus; existingEntityId?: string; firstSeenOrderIndex?: number; createdAt: string; updatedAt: string; }
export interface ProvisionalEntityMention { id: string; jobId: string; projectId: string; passageUnitId: string; chapterId: string; sceneId: string; startOffset: number; endOffset: number; excerpt: string; mentionText: string; resolvedProvisionalEntityId?: string; canonicalEntityId?: string; alternativeEntityIds: string[]; confidence: number; resolutionReason: string; createdAt: string; }
export interface ProvisionalAlias { id: string; provisionalEntityId: string; alias: string; sourceReferenceId?: string; confidence: number; reviewStatus: Exclude<ProvisionalReviewStatus, 'merged'>; createdAt: string; }
export interface ProvisionalRelation { id: string; jobId: string; projectId: string; sourceProvisionalEntityId: string; targetProvisionalEntityId: string; canonicalSourceEntityId?: string; canonicalTargetEntityId?: string; relationType: string; label: string; confidence: number; reviewStatus: Exclude<ProvisionalReviewStatus, 'merged'>; sourceReferenceId?: string; createdAt: string; updatedAt: string; }
export interface ProvisionalMergeProposal { id: string; jobId: string; projectId: string; leftProvisionalEntityId: string; rightProvisionalEntityId?: string; existingEntityId?: string; reason: string; confidence: number; reviewStatus: ProvisionalReviewStatus; createdAt: string; }
export interface ProvisionalEvent { id: string; jobId: string; projectId: string; passageUnitId: string; chapterId: string; sceneId: string; title: string; summary: string; participantEntityIds: string[]; canonicalParticipantEntityIds?: string[]; startOffset: number; endOffset: number; confidence: number; reviewStatus: Exclude<ProvisionalReviewStatus, 'merged'>; sourceReferenceId?: string; createdAt: string; }
export interface SaveProvisionalRelationInput extends Omit<ProvisionalRelation, 'id' | 'createdAt' | 'updatedAt'> { id?: string; }
export interface SaveProvisionalEventInput extends Omit<ProvisionalEvent, 'id' | 'createdAt'> { id?: string; }
export interface SaveProvisionalEntityInput extends Omit<ProvisionalEntity, 'id' | 'createdAt' | 'updatedAt'> { id?: string; }
export interface SaveProvisionalMentionInput extends Omit<ProvisionalEntityMention, 'id' | 'createdAt'> { id?: string; }
export interface SaveProvisionalMergeProposalInput extends Omit<ProvisionalMergeProposal, 'id' | 'createdAt'> { id?: string; }
export interface MaterializeProvisionalEntityInput { projectId: string; jobId: string; provisionalEntityId: string; existingEntityId?: string; decision: 'accept' | 'merge'; }
export interface ManuscriptEntityResolutionInput { projectId: string; jobId: string; unit: ManuscriptAnalysisUnit; passageText: string; previousContext: string; confirmedEntities: StoryEntity[]; previousProvisionalEntities: ProvisionalEntity[]; previousAliases: ProvisionalAlias[]; }
export interface ManuscriptEntityResolutionResult { entities: Array<{ temporaryId: string; entityType: ProvisionalEntityType; canonicalName: string; aliases: string[]; description: string; confidence: number; existingEntityId?: string }>; mentions: Array<{ mentionText: string; startOffset: number; endOffset: number; temporaryEntityId?: string; alternativeTemporaryIds: string[]; confidence: number; resolutionReason: string; excerpt: string }>; relations: Array<{ sourceTemporaryId: string; targetTemporaryId: string; relationType: string; label: string; confidence: number }>; events: Array<{ title: string; summary: string; participantTemporaryIds: string[]; startOffset: number; endOffset: number; confidence: number; excerpt: string }>; mergeProposals: Array<{ leftTemporaryId: string; rightTemporaryId?: string; existingEntityId?: string; reason: string; confidence: number }>; warnings: string[]; }
export type ManuscriptAnalysisPhaseStatus = 'pending' | 'running' | 'paused' | 'completed' | 'failed' | 'cancelled';
export interface ManuscriptAnalysisPhaseProgress { status: ManuscriptAnalysisPhaseStatus; totalUnits: number; completedUnits: number; failedUnits: number; requestedProvider: string; actualProvider?: string; inputHash?: string; outputHash?: string; lastSuccessfulUnitId?: string; currentUnitId?: string; currentLabel?: string; errorCode?: string; errorMessage?: string; updatedAt: string; }
export type ManuscriptAnalysisJobStatus = 'pending' | 'running' | 'paused' | 'awaiting_structure_review' | 'awaiting_user_review' | 'failed' | 'cancelled' | 'completed';
export interface ManuscriptAnalysisPageMarker { chapterId: string; pageNumber: number; label: string; sourceOffset: number; textOffset: number; }
export interface ManuscriptAnalysisJob { id: string; projectId: string; bookId: string; importReference: string; status: ManuscriptAnalysisJobStatus; currentPhase: ManuscriptAnalysisPhase; phaseProgress: Partial<Record<ManuscriptAnalysisPhase, ManuscriptAnalysisPhaseProgress>>; phaseErrors: Partial<Record<ManuscriptAnalysisPhase, { code: string; message: string; unitId?: string; at: string }>>; totalUnits: number; completedUnits: number; failedUnits: number; currentUnitId?: string; lastSuccessfulUnitId?: string; providerId: string; pageMarkers: ManuscriptAnalysisPageMarker[]; importVersionId?: string; createdAt: string; updatedAt: string; completedAt?: string; errorMessage?: string; }
export type ManuscriptAnalysisUnitStatus = 'pending' | 'running' | 'completed' | 'failed' | 'stale' | 'skipped';
export interface ManuscriptAnalysisUnit { id: string; jobId: string; projectId: string; chapterId: string; sceneId: string; orderIndex: number; pageNumber?: number; startOffset: number; endOffset: number; content: string; contentHash: string; status: ManuscriptAnalysisUnitStatus; retryCount: number; continuityRunId?: string; requestedProvider?: string; actualProvider?: string; promptVersion?: string; inputHash?: string; outputHash?: string; errorCode?: string; errorMessage?: string; createdAt: string; updatedAt: string; completedAt?: string; }
export interface CreateManuscriptAnalysisJobInput { projectId: string; bookId: string; importReference: string; providerId: string; pageMarkers?: ManuscriptAnalysisPageMarker[]; units: Array<{ id?: string; chapterId: string; sceneId: string; orderIndex: number; pageNumber?: number; startOffset: number; endOffset: number; content: string; contentHash: string; }>; newVersion?: boolean; }
export interface UpdateManuscriptAnalysisJobInput { id: string; status: ManuscriptAnalysisJobStatus; currentPhase?: ManuscriptAnalysisPhase; phaseProgress?: Partial<Record<ManuscriptAnalysisPhase, ManuscriptAnalysisPhaseProgress>>; phaseErrors?: Partial<Record<ManuscriptAnalysisPhase, { code: string; message: string; unitId?: string; at: string }>>; currentUnitId?: string; lastSuccessfulUnitId?: string; errorMessage?: string; completedAt?: string; }
...
export interface DetectBookGenreInput { projectId: string; bookId: string; chapterSummaries: NarrativeSummary[]; entities: StoryEntity[]; plotThreads: PlotThreadLifecycleProposal[]; confirmedRules: ProjectRule[]; timelineEvents: PersistentTimelineEvent[]; bookSynthesis?: ManuscriptSynthesisResult; projectStyle?: ProjectStyle; catalog?: Array<{ id: string; name: string; englishName?: string; category: string; description: string; searchTerms: string[]; active: boolean }>; }
export interface DetectBookGenreResult { primaryGenreId?: string; customPrimaryGenre?: string; secondaryGenreIds: string[]; customSecondaryGenres: string[]; confidence: number; reasoning: string; supportingSignals: string[]; contradictingSignals: string[]; alternativeGenres: Array<{ genreId?: string; customGenre?: string; reason: string }>; audienceNotes: string[]; warnings: string[]; }
export interface ManuscriptPhaseInput { projectId: string; bookId: string; chapters: Array<{ id: string; title: string; orderIndex: number; text: string }>; chapterSummaries: NarrativeSummary[]; confirmedEntities: StoryEntity[]; confirmedRules: ProjectRule[]; confirmedStates: ContinuityStateLedgerEntry[]; proposedFindings: ContinuityReviewFinding[]; proposedThreads: PlotThreadLifecycleProposal[]; contentHash: string; contextLevel?: 'hierarchical'; sceneContexts?: Array<{ id: string; chapterId: string; orderIndex: number; title: string; summary?: string; sourceReferenceIds: string[] }>; sourceReferences?: StorySourceReference[]; draftLedger?: ManuscriptAnalysisDraftLedgerEntry[]; timelineEvents?: PersistentTimelineEvent[]; budgetWarnings?: string[]; budget?: { maxCodePoints: number; includedCodePoints: number; truncatedSections: string[] }; }
...
export interface SavePlotThreadLifecycleInput extends Omit<PlotThreadLifecycle, 'id' | 'updatedAt'> { id?: string; }
export interface SavePlotThreadLifecycleProposalInput extends Omit<PlotThreadLifecycleProposal, 'id' | 'createdAt' | 'reviewedAt' | 'reviewStatus'> { id?: string; reviewStatus?: BibleReviewStatus; }
export interface ProjectContext { projectId: string; currentScene?: Scene; currentChapter?: Chapter; relevantEntities: StoryEntity[]; relevantSources: StorySourceReference[]; openPlotThreads: StoryEntity[]; possibleContradictions: StoryEntity[]; lore?: LoreMetadata[]; projectRules?: ProjectRule[]; entityRelations?: StoryEntityRelation[]; continuityStates?: ContinuityStateLedgerEntry[]; characterProfiles?: CharacterProfile[]; characterStates?: CharacterSceneState[]; characterVoicePatterns?: CharacterVoicePattern[]; characterExperiences?: CharacterExperience[]; characterDialogueMemories?: CharacterDialogueMemory[]; relationshipMemories?: RelationshipMemory[]; characterKnowledgeStates?: CharacterKnowledgeState[]; projectStyle?: ProjectStyle; styleReferences?: StyleReference[]; acceptedStyleObservations?: ProjectStyleObservation[]; narrativeSummaries?: NarrativeSummary[]; }
...
export interface CharacterMemoryUpdateRun { id: string; projectId: string; sceneId: string; contentHash: string; extractorId: string; manuscriptJobId?: string; analyzedContent?: string; status: BibleRunStatus; createdAt: string; completedAt?: string; errorMessage?: string; }
...
export interface CharacterMemoryExtractionInput { project: Project; chapter: Chapter; scene: Scene; characters: StoryEntity[]; existingEntities: StoryEntity[]; provisionalEntities?: ProvisionalEntity[]; provisionalAliases?: ProvisionalAlias[]; context: ProjectContext; changedRange?: { start: number; end: number }; }
...
export interface CreateCharacterMemoryUpdateRunInput { projectId: string; sceneId: string; contentHash: string; extractorId: string; manuscriptJobId?: string; analyzedContent?: string; }
...
export const LORE_SHEET_ITEM_TYPES = ['premise', 'world_rule', 'prerequisite', 'effect', 'limitation', 'cost', 'exception', 'term', 'organization', 'location', 'historical_event', 'known_aspect', 'unknown_aspect', 'rule_connection', 'open_question', 'warning'] as const;
export type LoreSheetItemType = typeof LORE_SHEET_ITEM_TYPES[number];
export type ExcludedContentDecisionStatus = 'pending' | 'routed' | 'ignored';
export interface LoreCrafterExcludedContent { content: string; suggestedTarget: LoreCrafterExcludedTarget; reason: string; }
export interface ExcludedContentDecision { id: string; runId: string; projectId: string; content: string; reason: string; suggestedTarget: LoreCrafterExcludedTarget; selectedTarget?: LoreCrafterExcludedTarget; decision: ExcludedContentDecisionStatus; createdArtifactId?: string; createdAt: string; updatedAt: string; }
export interface ProjectContentProposal { id: string; projectId: string; targetKind: LoreCrafterExcludedTarget; title: string; content: string; reason: string; sourceReferenceId?: string; origin: 'lore_crafter'; status: 'proposed' | 'accepted' | 'rejected' | 'uncertain'; createdAt: string; updatedAt: string; }
...
export interface LoreSheetWithItems { draft: LoreSheetDraft; items: LoreSheetItem[]; }
export interface LoreSheetItem { id: string; draftId: string; runId: string; projectId: string; itemType: LoreSheetItemType; title: string; content: string; confidence: number; structured?: Record<string, unknown>; sourceReferenceId?: string; targetEntityId?: string; targetRuleId?: string; status: LoreCrafterItemStatus; createdAt: string; updatedAt: string; }
export interface CreateLoreCrafterRunInput { projectId: string; originalText: string; contentHash: string; providerId: string; promptVersion: string; correctionText?: string; }
export interface UpdateLoreCrafterRunInput { id: string; status: LoreCrafterRunStatus; understandingSummary?: string; analysis?: LoreCrafterAnalysis; confirmationText?: string; errorCode?: string; errorMessage?: string; completedAt?: string; correctionText?: string; }
...
export interface ManuscriptAnalysisCompletionReport { id: string; jobId: string; projectId: string; contentHash: string; payload: { recognizedScenes: unknown[]; entities: unknown[]; merges: unknown[]; bibleDecisions: unknown[]; memoryDecisions: unknown[]; continuityFindings: unknown[]; timelineEvents: unknown[]; graphEdges: unknown[]; plotThreads: unknown[]; skippedResults: unknown[]; uncertainResults: unknown[]; rejectedResults: unknown[]; openOptionalResults: unknown[]; providers: string[]; promptVersions: string[]; warnings: string[]; truncations: string[]; }; createdAt: string; updatedAt: string; }
export interface SaveManuscriptAnalysisCompletionReportInput { id?: string; jobId: string; projectId: string; contentHash: string; payload: ManuscriptAnalysisCompletionReport['payload']; }
...
export interface CreateManuscriptImportPageMarkerInput { pageNumber: number; label: string; sourceOffset: number; textOffset: number; }
export interface CreateManuscriptImportUnitInput { pageNumber?: number; startOffset: number; endOffset: number; content: string; contentHash: string; }
export interface CreateManuscriptImportChapterInput extends ManuscriptImportChapterInput { pageMarkers: CreateManuscriptImportPageMarkerInput[]; units: CreateManuscriptImportUnitInput[]; }
export interface CreateManuscriptImportInput { projectId: string; bookId: string; originalText: string; originalContentHash: string; fileName: string; sourceKind?: 'external_text'; chapters: CreateManuscriptImportChapterInput[]; providerId: string; newVersion?: boolean; onboarding?: SaveProjectOnboardingStateInput; }
export interface ManuscriptImportVersion { id: string; projectId: string; bookId: string; sourceDocumentId: string; originalContentHash: string; versionNumber: number; status: 'active' | 'archived' | 'cancelled'; analysisJobId?: string; createdAt: string; activatedAt?: string; }
export interface ManuscriptImportWorkflowResult { importVersion: ManuscriptImportVersion; sourceDocument: ProjectSourceDocument; import: ManuscriptImportResult; analysisJob: ManuscriptAnalysisJob; }
...
export interface AcceptChapterGenerationJobInput { jobId: string; currentContextHash: string; }
...
export type ChapterGenerationDraftState = 'valid' | 'stale' | 'regenerate_requested';
export interface ChapterGenerationSection { id: string; jobId: string; planBeatId: string; orderIndex: number; targetWords: number; actualWords: number; content: string; contentHash?: string; draftContextHash?: string; draftState?: ChapterGenerationDraftState; continuationSummary: string; continuityState: DraftContinuityState; status: 'pending' | 'generating' | 'generated' | 'reviewed' | 'regenerate_requested' | 'failed'; providerId?: string; createdAt: string; updatedAt: string; }
export type SaveChapterGenerationSectionInput = Omit<ChapterGenerationSection, 'id' | 'actualWords' | 'createdAt' | 'updatedAt'>;
export interface ChapterGenerationReview { id: string; jobId: string; sectionId?: string; continuityRunId?: string; reviewScope: 'section' | 'chapter'; issueType: string; severity: 'info' | 'warning' | 'blocking'; title: string; description: string; relatedEntityIds: string[]; relatedSourceIds: string[]; suggestedAction: string; status: string; createdAt: string; updatedAt: string; }
export type SaveChapterGenerationReviewInput = Omit<ChapterGenerationReview, 'id' | 'createdAt' | 'updatedAt' | 'jobId'> & { jobId?: string };
export type ChapterGenerationDraftLedgerStatus = 'proposed' | 'superseded' | 'rejected' | 'accepted_for_manuscript_review';
export interface ChapterGenerationDraftLedgerEntry { id: string; jobId: string; sectionId: string; projectId: string; entityId: string; relatedEntityId?: string; stateKind: ContinuityStateKind; previousState: string; newState: string; sourceExcerpt: string; sourceStartOffset?: number; sourceEndOffset?: number; contentHash: string; confidence: number; status: ChapterGenerationDraftLedgerStatus; sourceReferenceId?: string; createdAt: string; updatedAt: string; }
export interface SaveChapterGenerationDraftLedgerInput extends Omit<ChapterGenerationDraftLedgerEntry, 'id' | 'createdAt' | 'updatedAt' | 'status'> { id?: string; status?: ChapterGenerationDraftLedgerStatus; }
