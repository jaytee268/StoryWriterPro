...
export function App() {
  ...
  const [manuscriptAnalysis, setManuscriptAnalysis] = useState<{ job: ManuscriptAnalysisJob; units: ManuscriptAnalysisUnit[]; draftLedger: import('./types/domain').ManuscriptAnalysisDraftLedgerEntry[]; phaseResults: ManuscriptAnalysisPhaseResult[]; artifacts: ManuscriptAnalysisArtifact[]; structureRuns: ManuscriptStructureRun[]; structureProposals: ManuscriptStructureProposal[] }>();
  ...
  const refreshManuscriptAnalysis = useCallback(async (jobId?: string) => {
    ...
    setManuscriptAnalysis({ job: progress.job, units: progress.units, draftLedger: progress.draftLedger, phaseResults: progress.phaseResults, artifacts: progress.artifacts, structureRuns, structureProposals });
  ...
  const reviewManuscriptArtifact = useCallback(async (id: string, status: ManuscriptAnalysisArtifact['reviewStatus'], explicitlySkipped = false) => { await repository.reviewManuscriptAnalysisArtifact(id, status, explicitlySkipped); await refreshManuscriptAnalysis(manuscriptAnalysis?.job.id); }, [manuscriptAnalysis?.job.id, refreshManuscriptAnalysis]);
