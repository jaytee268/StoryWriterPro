...
import { GuidedAnalysisOnboarding } from './features/onboarding/GuidedAnalysisOnboarding';
import { LoreCrafterView } from './features/story-bible/LoreCrafterView';
...
export function App() {
  ...
  const [bibleInitialTab, setBibleInitialTab] = useState<import('./features/story-bible/FoundationsView').FoundationTab | 'continuity' | 'lore_crafter' | 'reveals'>('bible');
  ...
  const [onboardingManuscriptText, setOnboardingManuscriptText] = useState('');
  ...
  const reviewManuscriptRevealFinding = useCallback(async (input: ReviewRevealComplianceFindingInput) => { try { await repository.reviewRevealComplianceFinding(input); } catch (error) { setManuscriptAnalysisError(error instanceof Error ? error.message : String(error)); } finally { await refreshManuscriptAnalysis(manuscriptAnalysis?.job.id); } }, [manuscriptAnalysis?.job.id, refreshManuscriptAnalysis]);
  ...
  const completeManuscriptAnalysisReview = useCallback(async (explicitlySkipOpen = false) => { const jobId = manuscriptAnalysis?.job.id; if (!jobId) return; const controller = manuscriptAnalysisController.current?.jobId === jobId ? manuscriptAnalysisController.current : new ManuscriptAnalysisController(repository, jobId); manuscriptAnalysisController.current = controller; try { await controller.completeUserReview(explicitlySkipOpen); const current = await repository.getProjectOnboardingState(manuscriptAnalysis.job.projectId); if (current.currentStep === 'manuscript') { await repository.saveProjectOnboardingState({ ...current, currentStep: 'completed', completedSteps: Array.from(new Set([...current.completedSteps, 'manuscript'])), skippedSteps: current.skippedSteps }); setOnboarding(undefined); void loadWorkspace(manuscriptAnalysis.job.projectId); } } catch (error) { setManuscriptAnalysisError(error instanceof Error ? error.message : String(error)); } finally { await refreshManuscriptAnalysis(jobId); } }, [loadWorkspace, manuscriptAnalysis, refreshManuscriptAnalysis]);
  ...
  const manuscriptProgressView = manuscriptAnalysis ? <ManuscriptAnalysisProgress job={manuscriptAnalysis.job} units={manuscriptAnalysis.units} draftLedger={manuscriptAnalysis.draftLedger} phaseResults={manuscriptAnalysis.phaseResults} artifacts={manuscriptAnalysis.artifacts} completionReport={manuscriptAnalysis.completionReport} reviewDetails={manuscriptAnalysis.reviewDetails} structureProposals={manuscriptAnalysis.structureProposals} chapters={workspace?.chapters ?? []} onReviewStructureProposal={async (id, status, manualChanges) => { await repository.reviewManuscriptStructureProposal(id, status, manualChanges); await refreshManuscriptAnalysis(manuscriptAnalysis.job.id); }} onApplyStructure={(runId) => void applyManuscriptStructure(runId)} onReviewArtifact={reviewManuscriptArtifact} onFindingDecision={applyManuscriptFindingDecision} onRevealFindingDecision={reviewManuscriptRevealFinding} onOpenRevealContract={() => { setBibleInitialTab('reveals'); setView('bible'); }} onReviewDraft={reviewManuscriptDraft} onOpenSource={(sourceId) => void openManuscriptSource(sourceId)} error={manuscriptAnalysisError} onResume={() => void startManuscriptAnalysis(manuscriptAnalysis.job.id)} onRetry={() => void retryManuscriptAnalysis()} onPause={() => void pauseManuscriptAnalysis()} onCancel={() => void cancelManuscriptAnalysis()} onCompleteReview={(skip) => void completeManuscriptAnalysisReview(skip)} /> : null;
  ...
  const guidedState = onboarding?.state && onboardingProject && onboarding.state.currentStep !== 'completed' ? onboarding.state : undefined;
  const handleGuidedLoreComplete = async () => {
    if (!onboardingProject) return;
    const saved = await repository.getProjectOnboardingState(onboardingProject.id);
    setOnboarding({ project: onboardingProject, state: saved });
    if (onboardingManuscriptText.trim()) setActiveModal('import');
  };
  const skipGuidedLore = async () => {
    if (!onboardingProject || !guidedState) return;
    const saved = await repository.saveProjectOnboardingState({ ...guidedState, currentStep: 'manuscript', completedSteps: Array.from(new Set([...guidedState.completedSteps, 'lore'])), skippedSteps: Array.from(new Set([...guidedState.skippedSteps, 'lore'])) });
    setOnboarding({ project: onboardingProject, state: saved });
    if (onboardingManuscriptText.trim()) setActiveModal('import');
  };
  const skipGuidedManuscript = async () => {
    if (!onboardingProject || !guidedState) return;
    await repository.saveProjectOnboardingState({ ...guidedState, currentStep: 'completed', completedSteps: Array.from(new Set([...guidedState.completedSteps, 'manuscript'])), skippedSteps: Array.from(new Set([...guidedState.skippedSteps, 'manuscript'])) });
    setOnboarding(undefined);
    await loadWorkspace(onboardingProject.id);
  };
  const handleImportedManuscript = async (workflow: import('./types/domain').ManuscriptImportWorkflowResult) => {
    const projectId = workspace?.project.id ?? onboardingProject?.id;
    setSelectedSceneId(workflow.import.scenes[0]?.id ?? '');
    setOnboardingManuscriptText('');
    setActiveModal(null);
    if (!projectId) return;
    await loadWorkspace(projectId);
    await refreshManuscriptAnalysis(workflow.analysisJob.id);
    if (onboarding?.state?.currentStep === 'manuscript') {
      const saved = await repository.getProjectOnboardingState(projectId);
      setOnboarding({ project: onboardingProject, state: saved });
      void startManuscriptAnalysis(workflow.analysisJob.id);
    } else {
      setProviderNotice(workflow.importVersion.versionNumber > 1 ? 'Neue Importversion gespeichert. Prüfe jetzt die Struktur.' : 'Import gespeichert. Die Analyse kann jetzt gestartet werden.');
    }
  };
  ...
  if (guidedState && onboardingProject) return <><GuidedAnalysisOnboarding project={onboardingProject} state={guidedState} loreNotes={onboardingLoreNotes} manuscriptText={onboardingManuscriptText} loreView={<LoreCrafterView projectId={onboardingProject.id} repository={repository} initialNotes={onboardingLoreNotes} onboardingMode onReviewComplete={() => void handleGuidedLoreComplete()} />} manuscriptProgress={manuscriptProgressView} hasManuscriptAnalysis={Boolean(manuscriptAnalysis)} onOpenImport={() => setActiveModal('import')} onSkipLore={() => void skipGuidedLore()} onSkipManuscript={() => void skipGuidedManuscript()} onAbort={() => { setOnboarding(undefined); void loadWorkspace(onboardingProject.id); }} />{activeModal === 'import' && workspace?.books[0] && <ManuscriptImportModal projectId={workspace.project.id} bookId={workspace.books[0].id} repository={repository} providerId={providerSettings.activeProvider} initialText={onboardingManuscriptText} onClose={() => setActiveModal(null)} onImported={handleImportedManuscript} />}</>;
  if (loadState.status === 'empty' || onboarding) return <div className="onboarding-screen"><ProjectOnboarding repository={repository} project={onboardingProject} state={onboarding?.state} onCreated={(createdProject, savedState, loreNotes, manuscriptText) => { setOnboarding({ project: createdProject, state: savedState }); setOnboardingLoreNotes(loreNotes ?? ''); setOnboardingManuscriptText(manuscriptText ?? ''); void loadWorkspace(createdProject.id); }} onContinue={(nextState) => { if (nextState.currentStep === 'completed') { setOnboarding(undefined); void loadWorkspace(nextState.projectId); } else setOnboarding((current) => current ? { ...current, state: nextState } : current); }} onOpenLore={() => { setBibleInitialTab('lore_crafter'); setOnboarding(undefined); void requestViewChange('bible'); }} onOpenImport={() => { setOnboarding(undefined); void openImport(); }} onAbort={() => { setOnboarding(undefined); void loadWorkspace(onboardingProject?.id); }} /></div>;
  ...
  return <div className={`app-shell simple-mode ${sidebarOpen ? 'sidebar-open' : 'sidebar-collapsed'} ${view === 'editor' ? 'writing-mode' : ''}`}>
    ...
    {activeModal === 'import' && workspace?.books[0] && <ManuscriptImportModal projectId={workspace.project.id} bookId={workspace.books[0].id} repository={repository} providerId={providerSettings.activeProvider} initialText={onboardingManuscriptText} onClose={() => setActiveModal(null)} onImported={handleImportedManuscript} />}
