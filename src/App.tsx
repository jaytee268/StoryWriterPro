...
import { createManuscriptAnalysisUnits, loadManuscriptAnalysisProgress, loadManuscriptAnalysisReviewDetails, ManuscriptAnalysisController } from './services/manuscriptAnalysis';
...
export function App() {
  ...
  return <div className={`app-shell simple-mode ${sidebarOpen ? 'sidebar-open' : 'sidebar-collapsed'} ${view === 'editor' ? 'writing-mode' : ''}`}>
    ...
    {activeModal === 'import' && workspace?.books[0] && <ManuscriptImportModal projectId={workspace.project.id} bookId={workspace.books[0].id} repository={repository} onClose={() => setActiveModal(null)} onResumeExisting={async (hash) => { const currentWorkspace = workspace!; const existing = (await repository.listManuscriptAnalysisJobs(currentWorkspace.project.id)).find((job) => job.importReference === hash || job.importReference.startsWith(`${hash}:version-`)); if (existing) { setActiveModal(null); await refreshManuscriptAnalysis(existing.id); setProviderNotice('Vorhandener Import geöffnet. Du kannst die Analyse fortsetzen.'); } }} onImported={async (result, ...args) => { const currentWorkspace = workspace!; const unitsByChapter = args[1] ?? []; const pageMarkersByChapter = args[2] ?? []; const sourceDocument = args[3]; const newVersion = args[4] ?? false; const units = createManuscriptAnalysisUnits(result.chapters, unitsByChapter, result.scenes); const pageMarkers = pageMarkersByChapter.flat(); const baseReference = sourceDocument?.contentHash ?? contentHash(result.chapters.map((chapter) => `${chapter.id}\n${chapter.scenes.map((scene) => canonicalizeSceneForAi(scene).text).join('\n')}`).join('\n')); const job = await repository.createManuscriptAnalysisJob({ projectId: currentWorkspace.project.id, bookId: currentWorkspace.books[0]!.id, importReference: baseReference, providerId: providerSettings.activeProvider, pageMarkers, units, newVersion }); const currentOnboarding = await repository.getProjectOnboardingState(currentWorkspace.project.id); await repository.saveProjectOnboardingState({ ...currentOnboarding, currentStep: 'summary', completedSteps: Array.from(new Set([...currentOnboarding.completedSteps, 'manuscript'])), importId: job.id }); setSelectedSceneId(result.scenes[0]?.id ?? ''); setActiveModal(null); await refreshManuscriptAnalysis(job.id); await loadWorkspace(currentWorkspace.project.id); setProviderNotice(newVersion ? 'Neue Importversion gespeichert. Prüfe jetzt die Struktur.' : 'Import gespeichert. Prüfe jetzt die Struktur, bevor du die Analyse startest.'); }} />}
