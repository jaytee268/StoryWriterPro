...
export function App() {
  ...
  const [manuscriptAnalysis, setManuscriptAnalysis] = useState<{ job: ManuscriptAnalysisJob; units: ManuscriptAnalysisUnit[]; phaseResults: ManuscriptAnalysisPhaseResult[]; artifacts: ManuscriptAnalysisArtifact[]; structureRuns: ManuscriptStructureRun[]; structureProposals: ManuscriptStructureProposal[] }>();
  ...
  const completeManuscriptAnalysisReview = useCallback(async () => { const jobId = manuscriptAnalysis?.job.id; if (!jobId) return; const controller = manuscriptAnalysisController.current?.jobId === jobId ? manuscriptAnalysisController.current : new ManuscriptAnalysisController(repository, jobId); manuscriptAnalysisController.current = controller; try { await controller.completeUserReview(true); } catch (error) { setManuscriptAnalysisError(error instanceof Error ? error.message : String(error)); } finally { await refreshManuscriptAnalysis(jobId); } }, [manuscriptAnalysis?.job.id, refreshManuscriptAnalysis]);
