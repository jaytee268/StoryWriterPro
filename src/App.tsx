...
export function App() {
  ...
  return <div className={`app-shell simple-mode ${sidebarOpen ? 'sidebar-open' : 'sidebar-collapsed'} ${view === 'editor' ? 'writing-mode' : ''}`}>
    ...
    {activeModal === 'import' && workspace?.books[0] && <ManuscriptImportModal projectId={workspace.project.id} bookId={workspace.books[0].id} repository={repository} providerId={providerSettings.activeProvider} initialText={onboardingManuscriptText} onClose={() => setActiveModal(null)} onImported={async (workflow) => { setSelectedSceneId(workflow.import.scenes[0]?.id ?? ''); setOnboardingManuscriptText(''); setActiveModal(null); await refreshManuscriptAnalysis(workflow.analysisJob.id); await loadWorkspace(workspace.project.id); setProviderNotice(workflow.importVersion.versionNumber > 1 ? 'Neue Importversion gespeichert. Prüfe jetzt die Struktur.' : 'Import gespeichert. Prüfe jetzt die Struktur, bevor du die Analyse startest.'); if (onboardingLoreNotes.trim()) { setBibleInitialTab('lore_crafter'); setView('bible'); } }} />}
