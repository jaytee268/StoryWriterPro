...
export function LoreCrafterView({ projectId, repository, initialNotes, onRunCreated }: Props) {
  ...
  const analyse = async () => {
    ...
    try { const { provider } = await providerRouter.getActiveProvider(); const next = await analyzeLoreDraft(repository, provider, { projectId, originalText: notes }); onRunCreated?.(next); const onboarding = await repository.getProjectOnboardingState(projectId); await repository.saveProjectOnboardingState({ ...onboarding, loreCrafterRunId: next.id }); await loadRun(next); setRuns(await repository.listLoreCrafterRuns(projectId)); setNotes(next.originalText); setMessage('Analyse abgeschlossen. Prüfe jetzt das Verständnis.'); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Die Lore konnte nicht analysiert werden.'); setMessage(''); } finally { setBusy(''); }
  };
  ...
  const finish = async () => { if (!run) return; setBusy('finish'); try { const finished = await finishLoreCrafterReview(repository, run, items); setRun(finished); const onboarding = await repository.getProjectOnboardingState(projectId); if (onboarding.currentStep === 'lore') await repository.saveProjectOnboardingState({ ...onboarding, currentStep: 'manuscript', completedSteps: Array.from(new Set([...onboarding.completedSteps, 'lore'])) }); setRuns(await repository.listLoreCrafterRuns(projectId)); setMessage('Review abgeschlossen. Übernommene Regeln bleiben bis zur Autorbestätigung inaktiv.'); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Review kann noch nicht abgeschlossen werden.'); } finally { setBusy(''); } };
