...
export function ProjectOnboarding({ repository, project, state, onCreated, onContinue, onOpenLore, onOpenImport }: Props) {
  ...
  if (!project) {
    const categories = Array.from(new Set(GENRE_CATALOG.map((entry) => entry.category))).sort();
    const visibleGenres = GENRE_CATALOG.filter((entry) => entry.active && (genreCategory === 'all' || entry.category === genreCategory) && (!genreQuery.trim() || `${entry.name} ${entry.description} ${entry.searchTerms.join(' ')}`.toLocaleLowerCase().includes(genreQuery.toLocaleLowerCase()))).slice(0, 18);
    return <section className="state-view onboarding-view"><span className="eyebrow">NEUES PROJEKT</span><h1>Dein Buch beginnt hier</h1><p>Lege ein leeres Projekt an. Du entscheidest anschließend selbst, ob du Lore oder Manuskript zuerst vorbereitest.</p><div className="onboarding-form"><label className="field-label">Projekttitel<input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Arbeitstitel" /></label><label className="field-label">Autor<input value={author} onChange={(event) => setAuthor(event.target.value)} placeholder="Dein Name" /></label><label className="field-label">Bandtitel <span>(optional)</span><input value={volumeTitle} onChange={(event) => setVolumeTitle(event.target.value)} placeholder="Wie der Projekttitel" /></label><div className="onboarding-two-columns"><label className="field-label">Sprache<select value={language} onChange={(event) => setLanguage(event.target.value)}><option value="de">Deutsch</option><option value="en">Englisch</option><option value="other">Andere</option></select></label><label className="field-label">Genre <span>(optional)</span><select value={primaryGenreId} onChange={(event) => { setPrimaryGenreId(event.target.value); setGenre(event.target.selectedOptions[0]?.text ?? ''); }}><option value="">Kein Hauptgenre</option>{GENRE_CATALOG.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label></div><div className="onboarding-genre-picker"><div className="onboarding-two-columns"><input value={genreQuery} onChange={(event) => setGenreQuery(event.target.value)} placeholder="Genre suchen" /><select value={genreCategory} onChange={(event) => setGenreCategory(event.target.value)}><option value="all">Alle Kategorien</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></div><small>Weitere Genres auswählen (optional)</small><div className="onboarding-genre-options">{visibleGenres.map((entry) => <label key={entry.id}><input type="checkbox" checked={secondaryGenreIds.includes(entry.id)} disabled={entry.id === primaryGenreId} onChange={(event) => setSecondaryGenreIds((current) => event.target.checked ? [...new Set([...current, entry.id])] : current.filter((id) => id !== entry.id))} /> {entry.name}</label>)}</div><label className="field-label">Eigenes Genre <span>(optional)</span><input value={customGenre} onChange={(event) => setCustomGenre(event.target.value)} placeholder="Eigene Bezeichnung" /></label></div>{error && <div className="save-error">{error}</div>}<button className="primary-button large" disabled={busy} onClick={() => void create()}><FolderPlus size={17} /> Projekt anlegen <ArrowRight size={17} /></button></div></section>;
  }
