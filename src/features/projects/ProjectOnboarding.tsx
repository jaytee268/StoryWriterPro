...
interface Props {
  ...
  onCreated: (project: Project, state: ProjectOnboardingState, loreNotes?: string, manuscriptText?: string) => void;
...
interface NewProjectOnboardingDraft { title: string; author: string; volumeTitle: string; language: string; genre: string; primaryGenreId: string; secondaryGenreIds: string[]; customGenre: string; customGenreOpen: boolean; loreNotes: string; manuscriptText: string; step: ProjectOnboardingStep; completedSteps: string[]; skippedSteps: string[]; }
const emptyNewProjectDraft: NewProjectOnboardingDraft = { title: '', author: '', volumeTitle: '', language: 'de', genre: '', primaryGenreId: '', secondaryGenreIds: [], customGenre: '', customGenreOpen: false, loreNotes: '', manuscriptText: '', step: 'project', completedSteps: [], skippedSteps: [] };
...
export function ProjectOnboarding({ repository, project, state, onCreated, onContinue, onOpenLore, onOpenImport, onAbort }: Props) {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(onboardingDraftStorageKey, JSON.stringify({ title, author, volumeTitle, language, genre, primaryGenreId, secondaryGenreIds, customGenre, customGenreOpen, loreNotes, manuscriptText, step, completedSteps: draftCompletedSteps, skippedSteps: draftSkippedSteps } satisfies NewProjectOnboardingDraft));
  }, [author, customGenre, customGenreOpen, draftAborted, draftCompletedSteps, draftSkippedSteps, genre, language, loreNotes, manuscriptText, primaryGenreId, project, secondaryGenreIds, step, title, volumeTitle]);
  ...
  const abort = () => {
    ...
    setTitle(''); setAuthor(''); setVolumeTitle(''); setLanguage('de'); setGenre(''); setPrimaryGenreId(''); setSecondaryGenreIds([]); setCustomGenre(''); setCustomGenreOpen(false); setLoreNotes(''); setManuscriptText(''); setDraftCompletedSteps([]); setDraftSkippedSteps([]); setStep('project'); setError('');
    onAbort?.();
  };
