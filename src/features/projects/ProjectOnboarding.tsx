...
interface NewProjectOnboardingDraft { title: string; author: string; volumeTitle: string; language: string; genre: string; primaryGenreId: string; secondaryGenreIds: string[]; customGenre: string; customGenreOpen: boolean; step: ProjectOnboardingStep; completedSteps: string[]; skippedSteps: string[]; }
const emptyNewProjectDraft: NewProjectOnboardingDraft = { title: '', author: '', volumeTitle: '', language: 'de', genre: '', primaryGenreId: '', secondaryGenreIds: [], customGenre: '', customGenreOpen: false, step: 'project', completedSteps: [], skippedSteps: [] };
...
export function ProjectOnboarding({ repository, project, state, onCreated, onContinue, onOpenLore, onOpenImport, onAbort }: Props) {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(onboardingDraftStorageKey, JSON.stringify({ title, author, volumeTitle, language, genre, primaryGenreId, secondaryGenreIds, customGenre, customGenreOpen, step, completedSteps: draftCompletedSteps, skippedSteps: draftSkippedSteps } satisfies NewProjectOnboardingDraft));
  }, [author, customGenre, customGenreOpen, draftAborted, draftCompletedSteps, draftSkippedSteps, genre, language, primaryGenreId, project, secondaryGenreIds, step, title, volumeTitle]);
  ...
  const abort = () => {
    ...
    setTitle(''); setAuthor(''); setVolumeTitle(''); setLanguage('de'); setGenre(''); setPrimaryGenreId(''); setSecondaryGenreIds([]); setCustomGenre(''); setCustomGenreOpen(false); setDraftCompletedSteps([]); setDraftSkippedSteps([]); setStep('project'); setError('');
    onAbort?.();
  };
