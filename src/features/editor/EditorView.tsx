...
import { ArrowLeft, Check, ChevronDown, FileDown, History, LoaderCircle, PanelLeftClose, PanelLeftOpen, Plus, Save, SpellCheck, StickyNote, Type, WandSparkles } from 'lucide-react';
...
export function EditorView({ chapters, scene, chapter, onBack, onSelectScene, onSave, onCreateChapter, onCreateScene, onListVersions, onRestoreVersion, onGetEditorPreferences, onSaveEditorPreferences, onSaveStateChange, onBibleUpdate, onResearch }: EditorProps) {
  ...
  return <section className="editor-view simple-editor">
    <div className="simple-editor-head"><div className="editor-heading"><button className="back-button" onClick={onBack}><ArrowLeft size={16} /> Zurück</button><span className="eyebrow">DU SCHREIBST IN</span><h1>{chapter?.title ?? 'Kapitel auswählen'}</h1><div className="scene-selector simple-scene-selector"><ChevronDown size={16} /> {draftScene?.title ?? 'Keine Szene'}</div><button className="outline-toggle" onClick={() => setOutlineOpen((open) => !open)} aria-expanded={outlineOpen} aria-controls="manuscript-outline">{outlineOpen ? <PanelLeftClose size={15} /> : <PanelLeftOpen size={15} />} {outlineOpen ? 'Kapitel einklappen' : 'Kapitel dauerhaft öffnen'}</button></div><div className="simple-editor-actions"><span className={`save-state save-state-${saveStatus}`}><Save size={15} /> {saveLabels[saveStatus]}</span><button className="ghost-button large" onClick={() => void openHistory()} disabled={!draftScene}><History size={17} /> Verlauf</button><button className="ghost-button large" onClick={() => void check()} disabled={!draftScene}><SpellCheck size={17} /> Text prüfen</button><button className="primary-button large" onClick={onBibleUpdate}><WandSparkles size={17} /> Story Bible aktualisieren</button></div></div>
    <div className={`editor-layout simple-editor-layout ${outlineOpen ? 'outline-open' : 'outline-collapsed'}`}><aside id="manuscript-outline" className={`document-tree simple-tree ${outlineOpen ? 'chapter-sidebar-open' : 'chapter-sidebar-collapsed'}`}>
      {!outlineOpen && <div className="chapter-rail" aria-label="Kapitelübersicht">{chapters.map((item) => <button key={item.id} className={`chapter-rail-number ${item.id === chapter?.id ? 'active' : ''}`} onClick={() => void selectChapter(item)} aria-label={`${item.title} öffnen`} title={item.title}>{String(item.orderIndex).padStart(2, '0')}</button>)}</div>}
