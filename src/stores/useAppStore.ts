...
export const useAppStore = create<AppState>((set) => ({ view: 'dashboard', sidebarOpen: true, inspectorOpen: false, focusMode: false, selectedEntityId: null, setView: (view) => set({ view }), toggleSidebar: () => set((state) => ({ sidebarOpen: !state.sidebarOpen })), toggleInspector: () => set((state) => ({ inspectorOpen: !state.inspectorOpen })), toggleFocusMode: () => set((state) => ({ focusMode: !state.focusMode })), selectEntity: (selectedEntityId) => set({ selectedEntityId }) }));
