...
export interface ManuscriptReviewArtifactDetail { artifactId: string; artifactType: ManuscriptAnalysisArtifactType; title: string; body: string; chapterId?: string; sceneId?: string; excerpt?: string; confidence?: number; reason?: string; sourceReferenceId?: string; startOffset?: number; endOffset?: number; supportingSignals?: string[]; contradictingSignals?: string[]; alternativeGenres?: string[]; audienceNotes?: string[]; warnings?: string[]; contractId?: string; }
...
export async function loadManuscriptAnalysisReviewDetails(repository: StoryRepository, progress: ManuscriptAnalysisProgress): Promise<ManuscriptReviewArtifactDetail[]> {
  ...
  for (const finding of revealFindings) add({ artifactId: finding.id, artifactType: 'reveal_compliance_finding', title: ({ premature_revelation: 'Geheimnis wird möglicherweise zu früh verraten', impossible_character_knowledge: 'Figur weiß hier möglicherweise zu viel', narrator_information_leak: 'Der Erzähler verrät verborgenes Wissen', forbidden_clue: 'Hinweis ist vor der Enthüllung zu eindeutig', reveal_plan_conflict: 'Text widerspricht dem geplanten Reveal', missing_required_foreshadowing: 'Erforderliche Vorbereitung fehlt möglicherweise', ambiguous_possible_leak: 'Möglicher Hinweis-Leak' } as Record<string, string>)[finding.findingType] ?? 'Reveal-Prüfung', body: finding.explanation, chapterId: finding.chapterId, sceneId: finding.sceneId, excerpt: finding.evidenceExcerpt, confidence: finding.confidence, reason: finding.explanation, sourceReferenceId: finding.sourceReferenceId, startOffset: finding.startOffset, endOffset: finding.endOffset, contractId: finding.contractId, audienceNotes: [`Leser: ${finding.expectedKnowledgeLevel}`, `Offenlegung: ${finding.actualDisclosureLevel}`] });
  ...
  for (const artifact of progress.artifacts.filter((item) => item.artifactType === 'genre_detection')) {
    const book = workspace.books.find((item) => item.id === artifact.artifactId);
    if (book) add({ artifactId: artifact.artifactId, artifactType: 'genre_detection', title: 'Vorgeschlagenes Buchgenre', body: [
      `Genres: ${[book.primaryGenreId, ...book.secondaryGenreIds, ...book.customGenreNames].filter(Boolean).join(', ') || '—'}`,
      book.genreSupportingSignals?.length ? `Positive Signale: ${book.genreSupportingSignals.join(' · ')}` : '',
      book.genreContradictingSignals?.length ? `Gegenargumente: ${book.genreContradictingSignals.join(' · ')}` : '',
      book.genreAlternatives?.length ? `Alternativen: ${book.genreAlternatives.map((alternative) => alternative.genreId ?? alternative.customGenre ?? '').filter(Boolean).join(' · ')}` : '',
      book.genreAudienceNotes?.length ? `Leserhinweise: ${book.genreAudienceNotes.join(' · ')}` : '',
      book.genreWarnings?.length ? `Warnungen: ${book.genreWarnings.join(' · ')}` : '',
    ].filter(Boolean).join('\n'), confidence: book.genreConfidence, reason: book.genreReason, supportingSignals: book.genreSupportingSignals, contradictingSignals: book.genreContradictingSignals, alternativeGenres: book.genreAlternatives?.map((alternative) => alternative.genreId ?? alternative.customGenre ?? '').filter(Boolean), audienceNotes: book.genreAudienceNotes, warnings: book.genreWarnings });
  }
...
export class ManuscriptAnalysisController {
  ...
  async completeUserReview(explicitlySkipOpen = false): Promise<void> {
    ...
    const openArtifacts = artifacts.filter((artifact) => artifact.reviewStatus === 'pending' && artifact.artifactType !== 'genre_detection');
    ...
    const reviewedArtifacts = await this.repository.listManuscriptAnalysisArtifacts(job.id);
    if (reviewedArtifacts.some((artifact) => artifact.reviewStatus === 'pending' && artifact.artifactType !== 'genre_detection')) throw new Error('Nach dem Überspringen sind noch offene Analyseartefakte vorhanden.');
    await this.repository.saveManuscriptAnalysisReviewAudit({ jobId: job.id, projectId: job.projectId, action: 'complete_review', artifactIds: reviewedArtifacts.map((artifact) => artifact.id), artifactTypes: [...new Set(reviewedArtifacts.map((artifact) => artifact.artifactType))] as ManuscriptAnalysisArtifactType[], note: explicitlySkipOpen ? 'Review mit ausdrücklich übersprungenen Ergebnissen abgeschlossen.' : 'Review aller jobgebundenen Ergebnisse abgeschlossen.' });
    ...
    const statusItems = reviewedArtifacts.map((artifact) => ({ artifactId: artifact.artifactId, artifactType: artifact.artifactType, reviewStatus: artifact.reviewStatus, explicitlySkipped: artifact.explicitlySkipped }));
      ...
      for (const contract of contracts) {
        ...
        try {
          ...
          if (findings.length) {
            ...
            await this.repository.saveManuscriptAnalysisArtifacts(job.id, savedFindings.map((finding) => ({ jobId: job.id, projectId: workspace.project.id, phase: 'reveal_compliance' as const, unitId: unit.id, artifactType: 'reveal_compliance_finding' as const, artifactId: finding.id, reviewStatus: 'pending' as const, explicitlySkipped: false })));
          }
  ...
  private async listPriorProvisionalEntities(jobId: string, orderIndex: number, includeCurrent = false): Promise<ProvisionalEntity[]> {
    ...
    return filterProvisionalEntitiesByOrder(entities, firstOrder, orderIndex, includeCurrent).map((entity) => ({ ...entity, firstSeenOrderIndex: firstOrder.get(entity.id) }));
  }
  ...
  private async runBookEndState(job: ManuscriptAnalysisJob, workspace: Awaited<ReturnType<StoryRepository['loadWorkspace']>>, provider: Provider, timeout: number): Promise<void> {
    ...
    if (!workspace.books.find((book) => book.id === job.bookId)?.genreAuthorConfirmed && typeof provider.detectBookGenre === 'function') {
      const genre: DetectBookGenreResult = await provider.detectBookGenre({ projectId: job.projectId, bookId: job.bookId, chapterSummaries: input.chapterSummaries, entities: input.confirmedEntities, plotThreads: input.proposedThreads, confirmedRules: input.confirmedRules, timelineEvents: input.timelineEvents ?? [], catalog: GENRE_CATALOG, bookSynthesis: { summary: result.summary, importantEvents: [], knowledgeChanges: result.knowledgeStates, relationshipChanges: result.relationships, openThreads: result.openActions, characterEndStates: result.characterEndStates, objectEndStates: result.objectOwners, injuries: result.injuries, unresolvedClues: result.unresolvedThreads, plotThreadStatuses: [], storyDirection: '', warnings: result.warnings } }, timeout);
      if (genre.primaryGenreId || genre.customPrimaryGenre || genre.secondaryGenreIds.length || genre.customSecondaryGenres.length) {
        await this.repository.saveBookGenres({ bookId: job.bookId, projectId: job.projectId, primaryGenreId: genre.primaryGenreId, secondaryGenreIds: genre.secondaryGenreIds, customGenreNames: [...(genre.customPrimaryGenre ? [genre.customPrimaryGenre] : []), ...genre.customSecondaryGenres], genreSource: 'ai_detected', genreConfidence: genre.confidence, genreReason: genre.reasoning, genreAuthorConfirmed: false, genreDetectedAt: new Date().toISOString(), genreSupportingSignals: genre.supportingSignals, genreContradictingSignals: genre.contradictingSignals, genreAlternatives: genre.alternativeGenres, genreAudienceNotes: genre.audienceNotes, genreWarnings: genre.warnings, genrePromptVersion: 'storymemory-genre-v1', analysisJobId: job.id, providerId: provider.id });
        await this.repository.saveManuscriptAnalysisArtifacts(job.id, [{ jobId: job.id, projectId: job.projectId, phase: 'book_end_state', artifactType: 'genre_detection', artifactId: job.bookId, reviewStatus: 'pending', explicitlySkipped: false }]);
      }
    }
  }
