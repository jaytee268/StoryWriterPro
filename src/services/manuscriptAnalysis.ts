...
export class ManuscriptAnalysisController {
  ...
  private async resolveProvisionalEntities(workspace: Awaited<ReturnType<StoryRepository['loadWorkspace']>>, job: ManuscriptAnalysisJob, unit: ManuscriptAnalysisUnit, passageText: string, previousContext: string, provider: Provider): Promise<void> {
    ...
    for (const entity of result.entities) {
      ...
      await this.repository.saveProvisionalEntity({ id, jobId: job.id, projectId: workspace.project.id, entityType: entity.entityType, canonicalName: entity.canonicalName, aliases: entity.aliases, description: entity.description, confidence: entity.confidence, existingEntityId: entity.existingEntityId && availableEntities.some((candidate) => candidate.id === entity.existingEntityId && candidate.projectId === workspace.project.id) ? entity.existingEntityId : undefined, reviewStatus: 'proposed' });
    }
    ...
    for (const [eventIndex, event] of result.events.entries()) {
      ...
      if (chapter && book) await this.repository.saveTimelineEvent({ id: `timeline-event-${job.id}-${unit.id}-${eventIndex}`, projectId: workspace.project.id, bookId: book.id, chapterId: unit.chapterId, sceneId: unit.sceneId, passageUnitId: unit.id, title: event.title, summary: event.summary, storyTimeText: chapter.scenes.find((scene) => scene.id === unit.sceneId)?.storyTime ?? '', temporalOrder: unit.orderIndex * 1_000_000 + startOffset, timeCertainty: chapter.scenes.find((scene) => scene.id === unit.sceneId)?.storyTime ? 'relative' : 'unknown', participatingEntityIds: participants, causeEventIds: [], consequenceEventIds: [], knowledgeChanges: [], stateChanges: [], relatedPlotThreadIds: [], sourceReferenceIds: source ? [source.id] : [], confidence: event.confidence, status: 'proposed', authorConfirmed: false, origin: 'manuscript_analysis' });
    }
