...
export async function loadManuscriptAnalysisReviewDetails(repository: StoryRepository, progress: ManuscriptAnalysisProgress): Promise<ManuscriptReviewArtifactDetail[]> {
  ...
  for (const artifact of progress.artifacts.filter((item) => item.artifactType === 'genre_detection')) {
    ...
    if (book) add({ artifactId: artifact.artifactId, artifactType: 'genre_detection', title: 'Vorgeschlagenes Buchgenre', body: [book.primaryGenreId, ...book.secondaryGenreIds, ...book.customGenreNames].filter(Boolean).join(', '), confidence: book.genreConfidence, reason: book.genreReason });
  }
