import type { Chapter, ContinuityAnalysisResult, ContinuityReviewFinding, ContinuityReviewSourceKind, ContinuityStateLedgerEntry, Project, ProjectRule, SaveContinuityFindingInput, SaveContinuityStateInput, StoryEntity, Scene, StorySourceReference, ContinuityCounterEvidence } from '../types/domain';
...
export async function runContinuityReview(repository: StoryRepository, input: ContinuityReviewRequest): Promise<{ runId: string; findings: ContinuityReviewFinding[]; stateProposals: ContinuityStateLedgerEntry[]; draftStateChanges: ContinuityAnalysisResult['proposedStateChanges']; analysis: ContinuityAnalysisResult }> {
  ...
  const providerInput: ContinuityAnalysisInput = { projectId: input.project.id, passage: { text: currentText, changedText: codepoints(currentText).slice(Math.max(0, (input.startOffset ?? incrementalWordRange(previousText, currentText).start) - passageStartOffset), Math.max(0, (input.endOffset ?? incrementalWordRange(previousText, currentText).end) - passageStartOffset)).join(''), chapterId: input.chapter?.id, sceneId: input.scene?.id, startOffset: input.startOffset, endOffset: input.endOffset, passageStartOffset, passageEndOffset, coordinateSystem: 'unicode_codepoints' }, previousContext: previousText ?? '', followingContext: followingText ?? '', confirmedStoryBible: confirmedEntities, confirmedLore: lore.filter((item) => candidateIds.has(item.entityId)).slice(0, 120), confirmedRules, continuityStatesBeforePosition: relevantStates, draftLedger: input.draftLedger ?? [], characterKnowledge: relevantKnowledge, characterProfiles: profiles.filter((profile): profile is NonNullable<typeof profile> => Boolean(profile)), characterMemories, activePlotThreads: confirmedEntities.filter((entity) => entity.type === 'plot_thread'), relevantSources, openFindings: openFindings.filter((finding) => finding.reviewStatus === 'open').slice(0, 100).map(({ id, objectiveConflict, reviewStatus }) => ({ id, objectiveConflict, reviewStatus })) };
  ...
  try {
    ...
    if (input.persistStateProposals !== false) {
      for (const item of draftStateChanges) stateProposals.push(await repository.saveContinuityStateEntry(await proposedStateInput(repository, input.project.id, input.chapter, input.scene, item)));
    }
    ...
    for (const change of result.plotThreadChanges) {
      ...
      await repository.savePlotThreadLifecycleProposal({ runId: run.id, projectId: input.project.id, entityId: change.entityId, proposedStatus: change.proposedStatus, evidenceExcerpt: change.evidenceExcerpt, sourceReferenceId, startOffset: change.startOffset ?? passageStartOffset, endOffset: change.endOffset ?? passageEndOffset, reason: change.reason });
    }
