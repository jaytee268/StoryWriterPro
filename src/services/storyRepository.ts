...
interface BrowserState { project: Project; books: Book[]; chapters: Chapter[]; entities: StoryEntity[]; versions: SceneVersion[]; editorPreferences: EditorPreferences; sources: StorySourceReference[]; runs: BibleUpdateRun[]; proposals: BibleProposal[]; lore: LoreMetadata[]; characterProfiles: CharacterProfile[]; characterStates: CharacterSceneState[]; projectStyle?: ProjectStyle; styleReferences: StyleReference[]; relations: StoryEntityRelation[]; }
...
export class BrowserDemoRepository implements StoryRepository {
  ...
  async createProject(input: CreateProjectInput): Promise<Project> {
    ...
    state.chapters = [];
    state.entities = [];
    state.versions = []; state.sources = []; state.runs = []; state.proposals = []; state.lore = []; state.characterProfiles = []; state.characterStates = []; state.projectStyle = undefined; state.styleReferences = []; state.relations = [];
    this.write(state);
