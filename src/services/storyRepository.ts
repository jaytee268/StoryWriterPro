...
const voiceSchema = z.object({ id: z.string(), projectId: z.string(), characterId: z.string(), relatedCharacterId: z.string().nullable().optional(), patternType: z.string(), patternText: z.string(), description: z.string(), contextCondition: z.string(), confidence: z.number(), status: memoryStatusSchema, authorConfirmed: z.boolean(), occurrenceCount: z.number(), createdAt: z.string(), updatedAt: z.string() }).transform((v) => ({ ...v, relatedCharacterId: v.relatedCharacterId ?? undefined }));
...
const memoryRunSchema = z.object({ id: z.string(), projectId: z.string(), sceneId: z.string(), contentHash: z.string(), extractorId: z.string(), status: z.enum(['pending', 'running', 'completed', 'failed', 'reviewed']), createdAt: z.string(), completedAt: z.string().nullable().optional(), errorMessage: z.string().nullable().optional() }).transform((v) => ({ ...v, completedAt: v.completedAt ?? undefined, errorMessage: v.errorMessage ?? undefined }));
const memoryProposalSchema = z.object({ id: z.string(), runId: z.string(), projectId: z.string(), sceneId: z.string(), proposalKind: z.string(), subjectCharacterId: z.string().nullable().optional(), relatedCharacterId: z.string().nullable().optional(), targetEntityId: z.string().nullable().optional(), payload: z.record(z.union([z.string(), z.number(), z.boolean(), z.null()])).optional().default({}), classification: z.string(), confidence: z.number(), evidenceExcerpt: z.string(), startOffset: z.number().nullable().optional(), endOffset: z.number().nullable().optional(), reason: z.string(), reviewStatus: z.enum(['pending', 'accepted', 'edited', 'rejected']), reviewedAt: z.string().nullable().optional(), createdAt: z.string() }).transform((v) => ({ ...v, subjectCharacterId: v.subjectCharacterId ?? undefined, relatedCharacterId: v.relatedCharacterId ?? undefined, targetEntityId: v.targetEntityId ?? undefined, startOffset: v.startOffset ?? undefined, endOffset: v.endOffset ?? undefined, reviewedAt: v.reviewedAt ?? undefined }));
...
export class BrowserDemoRepository implements StoryRepository {
  ...
  async reviewCharacterMemoryProposal(input: ReviewCharacterMemoryProposalInput): Promise<CharacterMemoryProposal> { const state = this.read(); const proposal = state.memoryProposals.find((x) => x.id === input.proposalId); if (!proposal) throw new Error('Character-Memory-Vorschlag nicht gefunden.'); if (proposal.reviewStatus !== 'pending') throw new Error('Dieser Vorschlag wurde bereits geprüft.'); const reviewed = { ...proposal, payload: input.payload ?? proposal.payload, reviewStatus: input.reviewStatus, reviewedAt: now() }; state.memoryProposals = state.memoryProposals.map((x) => x.id === proposal.id ? reviewed : x); this.write(state); return clone(reviewed); }
