  ...
  async extract(input: CharacterMemoryExtractionInput): Promise<CharacterMemoryExtractionResult> {
    ...
    for (const character of input.characters) {
      ...
      if (quoted) {
        ...
        proposals.push({ proposalKind: 'dialogue_memory', subjectCharacterId: character.id, payload: { dialogueKind: 'statement', topic: '', summary: `${character.name} spricht in dieser Szene.`, exactExcerpt: excerpt, emotionalTone: '', hiddenIntent: '', significance: 'supporting', truthfulness: 'unknown', participants: [{ characterId: character.id, role: 'speaker' }] }, classification: 'observable', confidence: 0.72, evidenceExcerpt: excerpt, startOffset: Array.from(text).slice(0, start).length, endOffset: Array.from(text).slice(0, start).length + Array.from(excerpt).length, reason: 'Explizite Sprecher-Markierung im Szenentext.' });
      }
    }
