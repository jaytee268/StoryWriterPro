...
describe('Runtime und Repository', () => {
  ...
  it('legt Versionen an und stellt einen älteren Stand wieder her', async () => { const repository = new BrowserDemoRepository(); const scene = await firstScene(repository); const first = await repository.updateScene({ ...scene, content: 'Erster Stand' }); await repository.updateScene({ ...first, content: 'Zweiter Stand' }); const versions = await repository.listSceneVersions(scene.id); expect(versions).toHaveLength(2); const restored = await repository.restoreSceneVersion(scene.id, versions[1]!.id); expect(restored.content).toBe('Erster Stand'); });
