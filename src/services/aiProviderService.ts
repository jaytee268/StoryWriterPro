...
const characterMemoryResultSchema = z.object({ proposals: z.array(z.object({ proposalKind: z.string(), subjectCharacterId: z.string().nullable().optional(), relatedCharacterId: z.string().nullable().optional(), targetEntityId: z.string().nullable().optional(), payload: z.record(z.union([z.string(), z.number(), z.boolean(), z.null()])).default({}), classification: z.string(), confidence: z.number().min(0).max(1), evidenceExcerpt: z.string(), startOffset: z.number().int().nonnegative().nullable().optional(), endOffset: z.number().int().nonnegative().nullable().optional(), reason: z.string() })), warnings: z.array(z.string()) });
...
export class LocalPrototypeProvider implements StoryAiProvider {
  ...
  async extractCharacterMemoryPatch(input: CharacterMemoryExtractionInput): Promise<CharacterMemoryExtractionResult> { return new LocalPrototypeCharacterMemoryExtractor().extract({ ...input, scene: canonicalizeSceneForAi(input.scene).scene }); }
