...
import type { AiProviderSettings, BibleExtractionInput, BibleExtractionResult, CharacterMemoryExtractionInput, CharacterMemoryExtractionResult, ChatSource, CodexCliCapabilities, ContinuityAnalysisResult, ContinuityStateLedgerEntry, GroundedChatResult, LoreMetadata, NarrativeSummaryAnalysisResult, ProjectContext, ProjectRule, ProjectStyle, ProjectStyleAnalysisResult, StoryEntity, StorySourceReference, CharacterProfile, CharacterExperience, CharacterDialogueMemory, RelationshipMemory, CharacterKnowledgeState, StyleReference } from '../types/domain';
...
const observedPassageSchema = z.object({ summary: z.string(), evidenceExcerpt: z.string(), entityIds: z.array(z.string()), startOffset: z.number().int().nonnegative().optional(), endOffset: z.number().int().nonnegative().optional() }).strict();
const stateChangeSchema = z.object({ entityId: z.string(), relatedEntityId: z.string().optional(), stateKind: z.enum(['item_existence', 'item_availability', 'ownership', 'location', 'physical_condition', 'injury', 'property', 'knowledge', 'relationship', 'promise', 'goal', 'open_action']), previousState: z.string(), newState: z.string(), confidence: z.number().min(0).max(1), evidenceExcerpt: z.string(), startOffset: z.number().int().nonnegative().optional(), endOffset: z.number().int().nonnegative().optional(), reason: z.string() }).strict();
const contradictionSchema = z.object({ findingType: z.enum(['critical_contradiction', 'probable_contradiction', 'missing_explanation', 'lore_compatible_anomaly', 'possible_intentional_exception']), subjectEntityId: z.string().optional(), relatedEntityIds: z.array(z.string()), relatedStateIds: z.array(z.string()), objectiveConflict: z.string(), evidenceExcerpt: z.string(), counterEvidenceExcerpts: z.array(z.string()), confidence: z.number().min(0).max(1), startOffset: z.number().int().nonnegative().optional(), endOffset: z.number().int().nonnegative().optional(), reason: z.string() }).strict();
const continuityResultSchema = z.object({ observedActions: z.array(observedPassageSchema), proposedStateChanges: z.array(stateChangeSchema), objectiveContradictions: z.array(contradictionSchema), missingExplanations: z.array(contradictionSchema), matchedLoreRules: z.array(z.object({ ruleId: z.string(), rationale: z.string(), confidence: z.number().min(0).max(1) }).strict()), newRuleProposals: z.array(z.object({ id: z.string().optional(), projectId: z.string(), targetRuleId: z.string().optional(), title: z.string(), statement: z.string(), scope: z.enum(['project', 'book', 'arc']), prerequisites: z.array(z.string()), effects: z.array(z.string()), exceptions: z.array(z.string()), connectedLoreIds: z.array(z.string()), sourceReferenceIds: z.array(z.string()), evidenceExcerpt: z.string(), chapterId: z.string().optional(), sceneId: z.string().optional(), startOffset: z.number().int().nonnegative().optional(), endOffset: z.number().int().nonnegative().optional(), confidence: z.number().min(0).max(1), reason: z.string() }).strict()), plotThreadChanges: z.array(z.object({ entityId: z.string(), proposedStatus: z.enum(['open', 'closure_candidate', 'partially_resolved', 'reopened', 'abandoned']), evidenceExcerpt: z.string(), startOffset: z.number().int().nonnegative().optional(), endOffset: z.number().int().nonnegative().optional(), reason: z.string(), confidence: z.number().min(0).max(1) }).strict()), confidence: z.number().min(0).max(1), evidence: z.array(z.object({ id: z.string(), label: z.string(), chapterId: z.string().optional(), sceneId: z.string().optional(), entityId: z.string().optional(), excerpt: z.string().optional(), startOffset: z.number().int().nonnegative().optional(), endOffset: z.number().int().nonnegative().optional() }).strict()), warnings: z.array(z.string()) }).strict();
...
export interface ContinuityAnalysisInput { projectId: string; passage: { text: string; changedText: string; chapterId?: string; sceneId?: string; startOffset?: number; endOffset?: number; }; previousContext: string; followingContext: string; confirmedStoryBible: StoryEntity[]; confirmedLore: LoreMetadata[]; confirmedRules: ProjectRule[]; continuityStatesBeforePosition: ContinuityStateLedgerEntry[]; draftLedger: ContinuityStateLedgerEntry[]; characterKnowledge: CharacterKnowledgeState[]; characterProfiles: CharacterProfile[]; characterMemories: Array<CharacterExperience | CharacterDialogueMemory | RelationshipMemory>; activePlotThreads: StoryEntity[]; relevantSources: StorySourceReference[]; openFindings: Array<{ id: string; objectiveConflict: string; reviewStatus: string }>; }
