...
export class DeterministicProjectContextBuilder implements ProjectContextBuilder {
  ...
  async build(input: ContextRequest): Promise<ProjectContext> {
    ...
    const [sources, lore, styleReferences, projectStyle, relations, allVoicePatterns, allExperiences, allDialogueMemories, allRelationshipMemories, allKnowledgeStates] = await Promise.all([this.repository.listSourceReferences(input.projectId), this.repository.getLoreMetadata(input.projectId), this.repository.listStyleReferences(input.projectId), this.repository.getProjectStyle(input.projectId), this.repository.listEntityRelations(input.projectId), this.repository.listCharacterVoicePatterns(input.projectId), this.repository.listCharacterExperiences(input.projectId), this.repository.listCharacterDialogueMemories(input.projectId), this.repository.listRelationshipMemories(input.projectId), this.repository.listCharacterKnowledgeStates(input.projectId)]);
    ...
    const relevantVoicePatterns = allVoicePatterns.filter((pattern) => characterIds.includes(pattern.characterId) || (pattern.relatedCharacterId ? characterIds.includes(pattern.relatedCharacterId) : false)).sort((a, b) => b.occurrenceCount - a.occurrenceCount).slice(0, 20);
    const relevantExperiences = allExperiences.filter((experience) => characterIds.includes(experience.characterId)).slice(0, 20);
    ...
    const relevantKnowledgeStates = allKnowledgeStates.filter((state) => characterIds.includes(state.characterId) && (!state.acquiredSceneId || (sceneOrder.get(state.acquiredSceneId) ?? 0) <= currentOrder) && (!state.changedSceneId || (sceneOrder.get(state.changedSceneId) ?? 0) <= currentOrder)).slice(0, 30);
