...
export class DeterministicProjectContextBuilder implements ProjectContextBuilder {
  ...
  async build(input: ContextRequest): Promise<ProjectContext> {
    ...
    const [sources, lore, styleReferences, projectStyle, relations, allVoicePatterns, allExperiences, allDialogueMemories, allRelationshipMemories, allKnowledgeHistory] = await Promise.all([this.repository.listSourceReferences(input.projectId), this.repository.getLoreMetadata(input.projectId), this.repository.listStyleReferences(input.projectId), this.repository.getProjectStyle(input.projectId), this.repository.listEntityRelations(input.projectId), this.repository.listCharacterVoicePatterns(input.projectId), this.repository.listCharacterExperiences(input.projectId), this.repository.listCharacterDialogueMemories(input.projectId), this.repository.listRelationshipMemories(input.projectId), this.repository.listCharacterKnowledgeHistory(input.projectId)]);
    ...
    const relevantKnowledgeStates = characterIds.flatMap((characterId) => {
      const history = allKnowledgeHistory.filter((state) => state.characterId === characterId).filter((state) => (!state.acquiredSceneId || (sceneOrder.get(state.acquiredSceneId) ?? Number.MAX_SAFE_INTEGER) < currentOrder) && (!state.changedSceneId || (sceneOrder.get(state.changedSceneId) ?? Number.MAX_SAFE_INTEGER) <= currentOrder)).sort((a, b) => (sceneOrder.get(a.changedSceneId ?? a.acquiredSceneId ?? '') ?? 0) - (sceneOrder.get(b.changedSceneId ?? b.acquiredSceneId ?? '') ?? 0));
      const latest = new Map<string, typeof history[number]>();
      history.forEach((state) => latest.set(state.factEntityId, state));
