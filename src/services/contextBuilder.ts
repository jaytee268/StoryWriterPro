...
export class DeterministicProjectContextBuilder implements ProjectContextBuilder {
  ...
  async build(input: ContextRequest): Promise<ProjectContext> {
    ...
    const sceneText = input.passageText ?? editorContentToPlainText(currentScene?.content ?? '');
    ...
    const passageEnd = input.passageEndOffset ?? Number.MAX_SAFE_INTEGER;
    const sourceIsAvailable = (source: StorySourceReference): boolean => source.sceneId !== currentScene?.id || source.startOffset === undefined || source.endOffset === undefined || source.endOffset <= passageEnd;
    const relevantSources = sources.filter((source) => sourceIsAvailable(source) && (source.entityId ? relevantEntityIds.has(source.entityId) : source.sceneId === currentScene?.id)).slice(0, 30);
