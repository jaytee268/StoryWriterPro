...
export class DeterministicProjectContextBuilder implements ProjectContextBuilder {
  ...
  async build(input: ContextRequest): Promise<ProjectContext> {
    ...
    const [sources, lore, styleReferences, projectStyle, relations, allVoicePatterns, allExperiences, allDialogueMemories, allRelationshipMemories, allKnowledgeStates, allKnowledgeHistory, styleObservations, narrativeSummaries] = await Promise.all([this.repository.listSourceReferences(input.projectId), this.repository.getLoreMetadata(input.projectId), this.repository.listStyleReferences(input.projectId), this.repository.getProjectStyle(input.projectId), this.repository.listEntityRelations(input.projectId), this.repository.listCharacterVoicePatterns(input.projectId), this.repository.listCharacterExperiences(input.projectId), this.repository.listCharacterDialogueMemories(input.projectId), this.repository.listRelationshipMemories(input.projectId), this.repository.listCharacterKnowledgeStates(input.projectId), this.repository.listCharacterKnowledgeHistory(input.projectId), this.repository.listProjectStyleObservations(input.projectId), this.repository.listNarrativeSummaries(input.projectId)]);
