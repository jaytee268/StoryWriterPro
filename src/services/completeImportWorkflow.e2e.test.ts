...
function fakeProvider(keeperId: string, threadId: string): StoryAiProvider {
  ...
  const provider: StoryAiProvider = {
    ...
    validateRevealCompliance: vi.fn(async () => ({ findings: [], warnings: [] })),
...
describe('vollständiger Erststart- und Manuskriptimport-Workflow', () => {
  ...
  it('führt den gesamten Fake-Provider-Workflow ohne Bulk-Skip aus', async () => {
    ...
    expect(result.finalUnits.map((unit) => unit.orderIndex).sort((left, right) => left - right)).toEqual(Array.from({ length: 12 }, (_, index) => index));
    for (const unit of result.finalUnits) {
      const chapter = result.finalWorkspace.chapters.find((item) => item.id === unit.chapterId)!;
      const scene = chapter.scenes.find((item) => item.id === unit.sceneId)!;
      const sceneText = Array.from(scene.content);
      expect(unit.startOffset).toBeGreaterThanOrEqual(0);
      expect(unit.endOffset).toBeLessThanOrEqual(sceneText.length);
      expect(unit.content).toBe(sceneText.slice(unit.startOffset, unit.endOffset).join(''));
    }
