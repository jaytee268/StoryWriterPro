...
pub fn initialize_connection(connection: &Connection) -> Result<()> {
    ...
    let has_multi_pass_manuscript_analysis: i64 = connection.query_row(
        "SELECT COUNT(*) FROM schema_migrations WHERE version = 21",
        [],
        |row| row.get(0),
    )?;
    if has_multi_pass_manuscript_analysis == 0 {
        connection.execute_batch(include_str!(
            "../../../migrations/021_multi_pass_manuscript_analysis.sql"
        ))?;
    ...
    let has_manuscript_structure_review: i64 = connection.query_row(
        "SELECT COUNT(*) FROM schema_migrations WHERE version = 26",
        [],
        |row| row.get(0),
    )?;
    if has_manuscript_structure_review == 0 {
        connection.execute_batch(include_str!(
            "../../../migrations/026_manuscript_structure_review.sql"
        ))?;
        connection.execute("INSERT INTO schema_migrations (version) VALUES (26)", [])?;
    }
    let has_provisional_graph: i64 = connection.query_row(
        "SELECT COUNT(*) FROM schema_migrations WHERE version = 27",
        [],
        |row| row.get(0),
    )?;
    if has_provisional_graph == 0 {
        connection.execute_batch(include_str!(
            "../../../migrations/027_provisional_manuscript_knowledge_graph.sql"
        ))?;
    ...
    let has_completion_reports: i64 = connection.query_row(
        "SELECT COUNT(*) FROM schema_migrations WHERE version = 31",
        [],
        |row| row.get(0),
    )?;
    if has_completion_reports == 0 {
        connection.execute_batch(include_str!(
            "../../../migrations/031_manuscript_analysis_completion_reports.sql"
        ))?;
    ...
    let has_transactional_imports: i64 = connection.query_row(
        "SELECT COUNT(*) FROM schema_migrations WHERE version = 34",
        [],
        |row| row.get(0),
    )?;
    if has_transactional_imports == 0 {
        connection.execute_batch(include_str!(
            "../../../migrations/034_transactional_manuscript_imports.sql"
        ))?;
        connection.execute("INSERT INTO schema_migrations (version) VALUES (34)", [])?;
    }
    let has_genre_detection_review: i64 = connection.query_row(
        "SELECT COUNT(*) FROM schema_migrations WHERE version = 35",
        [],
        |row| row.get(0),
    )?;
    if has_genre_detection_review == 0 {
        connection.execute_batch(include_str!(
            "../../../migrations/035_genre_detection_review.sql"
        ))?;
    ...
    let has_ai_setup: i64 = connection.query_row(
    ...
    let has_reveal_knowledge_contracts: i64 = connection.query_row(
        "SELECT COUNT(*) FROM schema_migrations WHERE version = 39",
        [],
        |row| row.get(0),
    )?;
    if has_reveal_knowledge_contracts == 0 {
        connection.execute_batch(include_str!(
            "../../../migrations/039_reveal_knowledge_contracts.sql"
        ))?;
